...
$.getScript('https://twscripts.dev/scripts/twSDK.js', async function () {
    ...
    if (isValidScreen && threadId) {
        ...
        UI.ErrorMessage(
            twSDK.tt('This script can only be run on an in-game forum thread!')
        );
    }
        ...
        jQuery('.text').mouseup(async function (event) {
            ...
            if (selectedText !== '') {
                if (
                    ...
                    jQuery('#raDefender').text(defendingTroopsHtml || 'N/A');
                    ...
                    embeds: [
                        {
                            ...
                            fields: [
                                ...
                                },
                                {
                                    ...
                                    value:
                                        jQuery('#raLoyalty').text().trim() ??
                                        'N/A',
                                    inline: false,
                                },
                                {
                                    name: twSDK.tt('Sigil'),
                                    value: `${villageEffects.sigil}%` ?? 'N/A',
                                    inline: false,
                                },
                                {
                                    name: twSDK.tt('Flag'),
                ...
                .then((response) => {
                    ...
                    if (tableRows.length) {
                        ...
                        tableRows.each(function () {
                            let dataTitle = jQuery(this).attr('title')?.trim();
    ...
    function parseVillageInfo(selectedText) {
        if (!selectedText.includes(twSDK.tt('Noble'))) {
            UI.ErrorMessage(
                twSDK.tt('No noble incomings found on the selected incomings!')
            );
            return {};
        } else {
            let linesOfText = selectedText.split('\n');
    ...
    function getVillageUnderAttack(selectedText) {
        ...
        const villageCoordinate = linesOfText[0].match(twSDK.coordsRegex)[0]; // returns 111|222
